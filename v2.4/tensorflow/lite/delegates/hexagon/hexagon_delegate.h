...
struct TFL_CAPI_EXPORT TfLiteHexagonDelegateOptions {
  ...
  /// hexagon_nn_init. Each graph corresponds to one delegated node subset in
  /// the TFLite model.
  ...
  /// input. input_batch_dimensions->data[i] is the index of the batch dimension
  /// for input[i]. If the graph has 1 input then the size of the array should
  /// be 1, and so on. This needs to be set only if 'enable_dynamic_batch_size'
  /// is true. Not needed for fixed graphs. If input[i] doesn't have dynamic
  /// batch, then input_batch_dimensions[i] should be -1. Delegate will take
  /// ownership of the pointer. WARNING: Experimental and subject to change
  /// anytime.
